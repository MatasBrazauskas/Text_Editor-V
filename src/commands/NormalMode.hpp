...
struct NormalModeCommand {
    NormalModeCommand(int count1, char operation, int count2, char motion, char textObject,
                     char targetChar, bool ignoreCount, ParsingStages stage): count1(count1), operation(operation), count2(count2),
    motion(motion), textObject(textObject), targetChar(targetChar), ignoreCount(ignoreCount), stage(stage){}
    int count1;
    int count2;
...
public:
    ...
    explicit NormalModeParser(const NormalModeTable&, int count1, char operation, int count2, char motion, char textObject,
...
public:
	...
	void executeNormalModeCommand(std::unique_ptr<ITextBuffer>& text, Cursor& cursor, EditorState& state, const NormalModeCommand command);
private:
