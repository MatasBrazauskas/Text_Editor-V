...
EditorConfig::EditorConfig(std::string title_t, const int tabSize_t, const bool autoSave_t,
			   const int autoSaveIntervalsMs_t, const int cursorBlinkMs_t,
			   const bool wrapText_t, const int fps_t, const bool verticalRuler_t,
			   const int verticalRulerCount_t)
    : title{std::move(title_t)}, tab_size{tabSize_t}, auto_save{autoSave_t},
      auto_save_intervals_ms(autoSaveIntervalsMs_t), cursor_blink_ms{cursorBlinkMs_t},
      wrap_text{wrapText_t}, fps{fps_t}, vertical_ruler{verticalRuler_t},
      vertical_ruler_count{verticalRulerCount_t} {}
...
ColorsConfig::ColorsConfig(const SDL_Color& backgroundColor_t, const SDL_Color& foregroundColor_t,
    ...
    : background_color{backgroundColor_t}, foreground_color{foregroundColor_t},
      cursor_color{cursorColor_t}, selection_color{selectionColor_t},
      vertical_ruler_color{verticalRulerColor_t} {}
...
SDL_Color ColorsConfig::getColorFromHex(std::string& hexStr) {
	...
	const bool allHexValues =
	    std::ranges::all_of(hexStr, [](char i) { return i >= '0' && i <= 'f'; });
	if (!allHexValues) {
		throw std::runtime_error("Invalid hex string.");
	}
...
ColorsConfig ColorsConfig::getColorConfig(const Json& json) {
	...
	return ColorsConfig{getColorFromHex(bgColorStr), getColorFromHex(frColorStr),
			    getColorFromHex(crColorStr), getColorFromHex(crFrColorStr),
			    getColorFromHex(vrColorStr)};
}
