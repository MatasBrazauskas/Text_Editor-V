...
ColorsConfig::ColorsConfig(const SDL_Color& backgroundColor, const SDL_Color& foregroundColor,
                           const SDL_Color& cursorColor, const SDL_Color&     selectionColor)
	: background_color(backgroundColor), foreground_color(foregroundColor),
	  cursor_color(cursorColor), selection_color(selectionColor) {
}
...
SDL_Color ColorsConfig::getColorFromHex(std::string& hexStr) {
	...
	bool allHexValues = std::ranges::all_of(hexStr, [](char i) {
		return i >= '0' && i <= 'f';
	});
	if (!allHexValues) {
		throw std::runtime_error("Invalid hex string.");
...
Config::Config(const Json& json) : editor_(EditorConfig::getEditorConfig(json)),
